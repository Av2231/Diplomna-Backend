...
public class LocationServiceImpl implements LocationService {
    ...
    @Override
    public ResponseEntity<?> addLocation(AddLocationRequest request) {
        ...
        Optional<Location> existing = locationRepository.findById(locationId);
    ...
    @Override
    public ResponseEntity<?> checkIfLocationExists(CheckLocationRequest request) {
    ...
    @Override
    public ResponseEntity<?> getAverageRatingForLocation(LocationIdRequest request) {
        if (request.getLocation_id() == null || request.getLocation_id().isBlank()) {
            return ResponseEntity.badRequest().body(Map.of(
                    "status", "failed",
                    "message", "Location ID was not provided!"
    ...
    @Override
    public ResponseEntity<?> findLocations(LocationSearchRequest req) {
        ...
        for (Location loc : allLocations) {
            ...
            for (Location.Availability av : loc.getAvailable_time()) {
                try {
                    ...
                    boolean sameDay =
                            fromDate.getYear() == avFrom.getYear() &&
                                    fromDate.getMonth() == avFrom.getMonth() &&
                                    fromDate.getDate() == avFrom.getDate();

                    boolean containsTimeRange =
                            !fromDate.before(avFrom) && !toDate.after(avTo);
