...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public ResponseEntity<?> changeProfilePicture(UpdatePictureRequest request) {
        ...
        if (optionalUser.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of("status", "failed", "message", "User is not existing!"));
        }
        ...
        userRepository.save(user);
        return ResponseEntity.ok(Map.of("status", "success", "message", "Picture saved!"));
    }

    @Override
    public ResponseEntity<?> addComment(AddCommentRequest request) {
        if (request.getUser_id() == null || request.getLocation_id() == null || request.getComment() == null) {
            return ResponseEntity.badRequest().body(Map.of("status", "failed", "message", "Parameters were not provided!"));
        }

        Optional<User> optionalUser = userRepository.findById(request.getUser_id());
        if (optionalUser.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of("status", "failed", "message", "User not found!"));
        }
        ...
        return ResponseEntity.ok(Map.of("status", "success", "message", "Commented successfully!"));
    }

    @Override
    public ResponseEntity<?> rateLocation(AddRatingRequest request) {
        if (request.getUser_id() == null || request.getLocation_id() == null) {
            return ResponseEntity.badRequest().body(Map.of("status", "failed", "message", "Some of the fields are empty!"));
        }
        ...
        userRepository.save(user);
        return ResponseEntity.ok(Map.of("status", "success", "message", "Location rated successfully!"));
    }

    @Override
    public ResponseEntity<?> getUserCommentScore(UserIdRequest request) {
        if (request.getUser_id() == null || request.getUser_id().isBlank()) {
            return ResponseEntity.badRequest().body(Map.of("status", "failed", "message", "User id was not provided!"));
        }
    ...
    @Override
    public ResponseEntity<?> getUserRatingScore(UserIdRequest request) {
        if (request.getUser_id() == null || request.getUser_id().isBlank()) {
            return ResponseEntity.badRequest().body(Map.of("status", "failed", "message", "User id was not provided!"));
        }
    ...
    @Override
    public ResponseEntity<?> hasUserRatedLocation(UserLocationCheckRequest request) {
        if (request.getUser_id() == null || request.getLocation_id() == null ||
                request.getUser_id().isBlank() || request.getLocation_id().isBlank()) {
            return ResponseEntity.badRequest().body(Map.of("status", "failed", "message", "User ID or location ID was not provided!"));
        }
    ...
    public ResponseEntity<?> getUserReservations(String userId) {
        ...
        User user = optionalUser.get();
        return ResponseEntity.ok(Map.of("status", "success", "reservations", user.getReservations()));
    }

    @Override
    public ResponseEntity<?> saveReservation(SaveReservationRequest request) {
        if (request.getUser_id() == null || request.getFrom_time() == null || request.getTo_time() == null) {
            return ResponseEntity.badRequest().body(Map.of("status", "failed", "message", "Missing required fields"));
        }
        ...
        return ResponseEntity.ok(Map.of("status", "success", "message", "Reservation saved successfully"));
    }
}